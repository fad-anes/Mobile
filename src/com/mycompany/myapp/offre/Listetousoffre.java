...
import gui.Freelancer;
import java.util.ArrayList;
/**
 *
 * @author anes_
 */
public class Listetousoffre extends BaseForm{
   Form current; 
public Listetousoffre(Resources res ) {
 ...
 super("Newsfeed", BoxLayout.y());
        ...
        ArrayList<Offre>list = ServiceOffre.getInstance().Affichetous();
...
Button searchButton = new Button("", FontImage.createMaterial(FontImage.MATERIAL_SEARCH, UIManager.getInstance().getComponentStyle("Button")));

// Ajouter une action à exécuter lorsque le bouton est cliqué
searchButton.addActionListener((e) -> {
    // Récupérer le texte de recherche
    String searchTerm = searchField.getText();
    // Afficher les résultats de recherche dans une nouvelle fenêtre
    serchoffre a = new serchoffre(res, searchTerm);
    a.show();
});

// Créer un conteneur pour empiler le champ de recherche et le bouton
Container searchContainer = LayeredLayout.encloseIn(
    BorderLayout.center(searchField),
    BorderLayout.east(searchButton)
);

// Ajouter le conteneur à la barre de navigation
getToolbar().setTitleComponent(searchContainer);    
