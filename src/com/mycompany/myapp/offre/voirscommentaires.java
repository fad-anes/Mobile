 ...
 * @author anes_
 */
public class voirscommentaires extends BaseForm{
     Form current; 
public voirscommentaires(Resources res,int ido ) {
...
Button viewMycommentairesBtn = new Button();
viewMycommentairesBtn.setIcon(FontImage.createMaterial(FontImage.MATERIAL_VISIBILITY, UIManager.getInstance().getComponentStyle("Button")));
viewMycommentairesBtn.setText("mes commentaires");
viewMycommentairesBtn.setUIID("SelectBar");

viewMycommentairesBtn.addActionListener((e) -> {
    InfiniteProgress ip = new InfiniteProgress();
    final Dialog ipDlg = ip.showInifiniteBlocking();
        
    mescommentaire a = new mescommentaire(res,ido);
    a.show();
    refreshTheme();
});
...
Container container = GridLayout.encloseIn(1, viewMyOffersBtn,viewMycommentairesBtn,ajoutercmm);
...
container.getAllStyles().setMargin(Component.RIGHT, 10);
 ...
 add(LayeredLayout.encloseIn(container));   
  ...
  private void addButton(Image img, Commentaire com, Resources res) {
